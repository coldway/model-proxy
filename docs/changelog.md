...
- **聊天内置工具 — 自然语言模型管理**
  ...
  - `list_provider_models`：通过聊天查询厂商模型（如"groq 支持哪些模型"）
    - 调用厂商 API 拉取全量模型列表
    - 分三组返回：未添加 / 已启用 / 已禁用
  - `search_ollama_library`：搜索 Ollama 模型库并按本机硬件筛选
    - 抓取 ollama.com/search 解析模型名称、能力标签、参数大小、下载量
    - 根据 GPU 显存（默认 12GB）估算 Q4 量化 VRAM 占用，分级评估适合度
    - 标记本地已安装模型，区分"推荐/可用/不适合"三档
    - 支持查询如"适合本机的 abliterated 模型"等自然语言场景
...
- **Session 持久化非原子写入**：改为写 `.tmp` 临时文件 + `os.replace()` 原子重命名，防止进程崩溃时文件截断
